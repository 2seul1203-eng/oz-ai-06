...
def get_opeai_client(request: Request):
    return request.app.state.client

# 우리가 원하는 OpenAI 대답 형식
...
@app.post(
    "/chat-gpt",
    summary="ChatGPT APT 호출",
)
async def chat_gpt_handler(
    openai_client = Depends(get_opeai_client),
    ...
    async def token_generator():
        async with openai_client.responses.stream(
            model="gpt-4.1-mini",
            input=[
                {"role": "system", "content": "한 줄로 짧게 답변"},
            ...
            async for event in stream:
                if event.type == "response.output_text.delta":
                    yield event.delta # token 단위로 스트리밍
